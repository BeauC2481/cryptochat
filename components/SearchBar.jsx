...
const SearchBar = ({ activeSelect, setActiveSelect, handleSearch, clearSearch }) => {
  ...
  return (
    <>
      <div className="flexCenter w-[70%] minmd:w-[60%] minlg:w-[50%] dark:bg-nft-black-2 bg-white border dark:border-nft-black-2 border-nft-gray-2 py-3 px-4 rounded-md">
        <Image
          src={images.search}
          objectFit="contain"
          width={20}
      ...
      >
        ...
        {toggle && (
          <div className="absolute top-full left-0 right-0 w-full mt-3 z-10 dark:bg-nft-black-2 bg-white border dark:border-nft-black-2 border-nft-gray-2 py-3 px-4 rounded-md">
            {['Recently posted', 'Tips (low to high)', 'Tips (high to low)'].map((item) => (
              <p
